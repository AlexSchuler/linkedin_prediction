...
class ClassifierRunner():
    def __init__(self,mlc: MultiLabelClassifier , batch_size: int, num_workers: int, input_dim: int,  ):
        self.mlc = mlc
    ...
    def train(self, partial_dataset: PartialDataset, epoch: int) -> None:
        ...
        batch_cnt = 0
        train_dataloader = DataLoader(dataset=partial_dataset, batch_size=self.batch_size,
        ...
        for encoded_features, textblocks, y in train_dataloader:
            ...
            embedded_textblocks = [self.bert(textblock) for textblock in
                                   tokenized_textblocks]  # TODO: Remove slice to two
            X = concat(encoded_features + embedded_textblocks, perform__pad=True,
            ...
            if ((batch_cnt) %5)==0:
                wandb.log(
                {'Epoch': epoch, 'Loss': loss.data }, step=self.example_cnt)
    ...
    def _tokenize_textblocks(self, textblocks: list):
        return [self.tokenizer(seq, truncation=True, padding="max_length", max_length=512, return_tensors="pt",) for seq in textblocks]

def _get_dim(tensors: list[torch.Tensor]):
