...
class PartialDataset(Dataset):
    ...
    def _prepare_dataset(self) -> None:
        self.onh_encodings, self.ord_encodings = self.preprocessor.construct_encodings(self.X)
        self.num_features = self.preprocessor.preprocess_numerical(self.X, self.dataset_type)
        to_drop = (
            self.preprocessor.ordinal_columns + self.preprocessor.numerical_columns + self.preprocessor.one_hot_columns
        )
        self.X = self.X.drop(to_drop, axis=1)

    def __getitem__(self, idx: int) -> tuple[list[torch.Tensor, torch.Tensor, torch.Tensor], list[dict], torch.Tensor]:
